...
def gpt_summary(query,model,language):
    if language == "zh":
        messages = [
            ...
            {"role": "assistant", "content": f"請用中文總結這篇文章，先提取出{keyword_length}個關鍵詞，在同一行內輸出，然後換行，用中文在{summary_length}字內寫一個包含所有要點的總結，按順序分要點輸出，並按照以下格式輸出'<br><br>總結:'，<br>是HTML的換行符，輸出時必須保留2個，並且必須在'總結:'二字之前"}
        ]
    else:
        messages = [
            {"role": "user", "content": query},
